...
class ChatInputView: View, Notifable {
    ...
    public func textViewEnterPressed(_ event: NSEvent) -> Bool {
        ...
        if FastSettings.checkSendingAbility(for: event) {
            ...
            if text.lowercased().starts(with: "/gemini ") {
                let prompt = String(text.dropFirst("/gemini ".count)).trimmed
                interaction.clearInput()
                interaction.updateInput(with: "Gemini is thinking...")
                GeminiService.shared.generateContent(prompt: prompt) { response in
                    DispatchQueue.main.async {
                        interaction.clearInput()
                        interaction.updateInput(with: response ?? "Error generating AI content.")
                    }
                }
                return true
            }

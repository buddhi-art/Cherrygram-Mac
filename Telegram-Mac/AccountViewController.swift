...
private enum AccountInfoEntry : TableItemListNodeEntry {
    ...
    case cherrygramSettings(index: Int, viewType: GeneralViewType)
    ...
    var index:Int {
        ...
        case let .attach(index, _, _):
            return index
        case let  .cherrygramSettings(index, _):
            return index
    ...
    func item(_ arguments: AccountInfoArguments, initialSize: NSSize) -> TableRowItem {
        ...
        case let .cherrygramSettings(_, viewType):
            return GeneralInteractedRowItem(initialSize, stableId: stableId, name: "Cherrygram Settings", icon: theme.icons.settingsGeneral, activeIcon: theme.icons.settingsGeneralActive, type: .next, viewType: viewType, action: {
                arguments.presentController(CherrygramSettingsViewController(arguments.context), true)
...
//    let normalCount = accounts.filter({ !$0.peer.isPremium }).count
    ...
    entries.append(.cherrygramSettings(index: index, viewType: .singleItem))
    index += 1
